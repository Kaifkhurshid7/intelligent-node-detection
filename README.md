...
```json
{
  ...
  "data": {
    ...
    "timings": {
      "preprocessing_ms": 58.9,
      "detection_ms": 21.8,
      "ocr_ms": 19.3,
      "merging_ms": 1.1,
      "classification_ms": 0.2,
      "filtering_ms": 0.1,
      "edge_detection_ms": 14.5,
      "graph_construction_ms": 0.4,
      "total_ms": 116.3
    }
  }
}
...
Health check with dependency status for container orchestration.
...
|---------|---------------|
| **API Versioning** | Routes mounted at both `/` and `/api/v1/` for backward compatibility |
| **Request Tracing** | UUID-based request IDs in `X-Request-ID` header for observability |
| **Performance Profiling** | Per-stage timing instrumentation returned in every response |
| **Typed Schemas** | Pydantic models for all API contracts with auto-generated OpenAPI docs |
| **Custom Exceptions** | Typed error hierarchy (`ValidationError`, `PipelineError`, etc.) with global handlers |
| **Dependency Health** | `/health` reports OCR and NLP availability for degraded-mode awareness |
| **Async File Handling** | Non-blocking file reads with size validation before processing |
| **Application Factory** | `create_app()` pattern for testability and configuration injection |
| **Structured Logging** | Consistent format with request IDs, timing, and log levels |
